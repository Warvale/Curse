...
public class Lobby {
	...
	private void checkIfSchematicCanBePasted() {
		...
		if (UHCMeetup.getInstance().getLobbySchematic().exists()) {
			useSchematic = true;
		} else {
			useSchematic = false;
		}
	...
	public void build(){
		if(!built && useSchematic){
			...
			try {
				...
				dimensions = SchematicHandler.doPaste(loc, UHCMeetup.getInstance().getDataFolder() + File.separator +  UHCMeetup.getInstance().getLobbySchematic().getName());
				Lobby.height = dimensions.get(0);
				Lobby.length = dimensions.get(1);
				Lobby.width = dimensions.get(2);
				built = true;
			} catch (Exception e) {
				Bukkit.getLogger().severe("An error occurred while pasting the lobby");
				e.printStackTrace();
				built = false;
			}
		}
